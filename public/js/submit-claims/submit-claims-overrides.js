  ...
  $(getProjectsTableList()).each(function (i) {
    ...
    if (isSelected(selected)) {
      ...
      projectCode.textContent = $(
        $(this).find("td").get(projectsColumnPositions.Project_Code),
      ).text();
      ratingCode.textContent = $($(this).find("td").get(projectsColumnPositions.Rating)).attr(
        "rating",
      );
      noReceipts.textContent = $($(this).find("td").get(projectsColumnPositions.Receipts))
        .find("input")
        .attr("value");
      noMissingReceipts.textContent = $($(this).find("td").get(projectsColumnPositions.No_Receipts))
        .find("input")
        .attr("value");
      ...
      if (includeItemList) {
        ...
        $(getClaimItems()).each(function (i) {
          ...
          if (
            projectKey.textContent == $(claimProjectKey).text() &&
            $(selected).find("input").attr("checked")
          ) {
            itemNode = xmlProjectList.createElement("Item");
            itemNode.textContent = $(
              $(claimItem).find("td").get(claimsColumnPositions.Item_ID),
            ).text();

            items.appendChild(itemNode);
            claim.appendChild(items);
            driver_survey = $(
              $(claimItem).find("td").get(claimsColumnPositions.DRIVER_SURVEY),
            ).text();
            if (driver_survey == "Y") {
