...
function forEachBankHolidayCell(
  myBankHolidays: BankHolidayEvent[],
    ...
    if (selectedDateObj) {
      ...
      for (let intCnt = 1; intCnt < 32; intCnt += 1) {
        const obj = document.getElementById(
          strCalendarDayPrefix + intCnt.toString(),
...
export function getBankHolidaysJSON(
  ...
  const cacheAvailable =
    settings.cacheDateBankHolidays && currentDate - thirtyDays < settings.cacheDateBankHolidays;
  ...
  if (cacheAvailable) {
    ...
    fetchBankHolidaysJSON((holidaysJSON: BankHolidayResponse) => {
      ...
      try {
        if (!settings.holidayRegion) throw Error("Your bank holiday region is unset!");
        if (!holidaysJSON)
