...
function injectCustomButtonsContainer() {
  // Get menu bar
  const buttonRow = document.querySelector<HTMLTableRowElement>(
...
function loadSelectMode(defaultMode: boolean, selectHours: number) {
  ...
  const checkboxes = inputs.map((input) => {
    ...
    if (input.parentElement !== null) {
      input.parentElement.ondragstart = function ondragstart() {
        return false;
      };
    }
    ...
    // checkboxes
    checkbox.parentNode?.addEventListener(
      "contextmenu",
      (event) => {
  ...
  function changeSelectMode(enabled: boolean) {
    ...
    checkboxes.forEach((combo) => {
      combo.style.display = enabled ? "block" : "none";
    });
    inputs.forEach((combo) => {
      combo.style.display = enabled ? "none" : "block";
    });
...
function setLeftButtonState(e: MouseEvent) {
  lmbDown = e.buttons === undefined ? e.which === 1 : e.buttons === 1;

  rmbDown = e.buttons === undefined ? e.which === 3 : e.buttons === 2;
